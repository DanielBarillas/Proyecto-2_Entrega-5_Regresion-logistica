...
train_data$es_cara <- factor(ifelse(train_data$SalePriceCat == "cara", "caro", "no_caro"))
test_data$es_cara <- factor(ifelse(test_data$SalePriceCat == "cara", "caro", "no_caro"))


predictors <- c("OverallQual", "GrLivArea", "GarageCars", "TotalBsmtSF", "YearBuilt")


learning_curve_fixed_test <- function(train_data, test_data, prop_seq = seq(0.1, 1, by = 0.1)) {
  results <- data.frame(TrainProp = numeric(), TrainError = numeric(), TestError = numeric())

  set.seed(123)
  for (p in prop_seq) {
    
    idx <- sample(1:nrow(train_data), size = floor(p * nrow(train_data)))
    train_split <- train_data[idx, ]

    # Entrenar modelo
    modelo <- train(as.formula(paste("es_cara ~", paste(predictors, collapse = "+"))),
                    data = train_split,
                    method = "glm",
                    family = "binomial",
    ...
    # Predicciones
    pred_train <- predict(modelo, newdata = train_split)
    pred_test <- predict(modelo, newdata = test_data)

    # Calcular errores
    err_train <- mean(pred_train != train_split$es_cara)
    err_test <- mean(pred_test != test_data$es_cara)

    results <- rbind(results, data.frame(TrainProp = p, TrainError = err_train, TestError = err_test))
  }

  return(results)
}

curva <- learning_curve_fixed_test(train_data, test_data)

ggplot(curva, aes(x = TrainProp)) +
  geom_line(aes(y = TrainError, color = "Entrenamiento")) +
  geom_line(aes(y = TestError, color = "Prueba")) +
  labs(title = "Curva de aprendizaje (test fijo)",
...
# Definir los porcentajes del conjunto de entrenamiento a utilizar
porcentajes <- seq(0.1, 1, by = 0.1)
resultados <- data.frame(Porcentaje = numeric(), Acc_train = numeric(), Acc_test = numeric())

# Iterar sobre los diferentes porcentajes de datos de entrenamiento
for (p in porcentajes) {
  ...
  # Asegurarse de que la variable 'es_cara' esté presente en train_parcial
  train_parcial$es_cara <- factor(ifelse(train_parcial$SalePriceCat == "cara", "caro", "no_caro"))
  
  # Entrenar el modelo con los datos de entrenamiento parciales
  modelo_rl <- train(
    es_cara ~ OverallQual + GrLivArea + GarageCars + TotalBsmtSF + YearBuilt,
    data = train_parcial,
    method = "glm",
...
ggplot(resultados, aes(x = Porcentaje)) +
  geom_line(aes(y = Acc_train, color = "Entrenamiento"), size = 1) +
  geom_line(aes(y = Acc_test, color = "Prueba"), size = 1) +
  geom_point(aes(y = Acc_train, color = "Entrenamiento")) +
  geom_point(aes(y = Acc_test, color = "Prueba")) +
  labs(
    title = "Curva de Aprendizaje: Comparación entre Entrenamiento y Prueba",
    x = "Porcentaje del conjunto de entrenamiento",
